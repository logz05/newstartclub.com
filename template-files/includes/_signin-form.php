	<div class="grid12 clearafter">
		<div class="left">
			{if "{embed:title}" !=""}<h1 class="heading">{embed:title}</h1>{if:elseif "{embed:title}" ==""}<h1>Sign In</h1>{/if}
			{if "{embed:msg}" !=""}<p>{embed:msg}</p>
			{/if}{exp:member:login_form error_page="members/error" return="{site_url}{segment_1}/{segment_2}/{segment_3}/{segment_4}/{segment_5}/{segment_6}/{segment_7}"}
			<table>
				<tr>
