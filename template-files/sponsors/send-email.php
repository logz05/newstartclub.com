...
{if (segment_3 == 'interest' || segment_3 == 'more-info') && segment_4}{!-- Category Member Listing --}
	{exp:user:stats dynamic="off"}
		{embed="sponsors/_email-interest" sponsor_number="{member_admin_id}" sponsor_zipcode="{sponsor_zipcode}"}
	{/exp:user:stats}
	
{if:elseif segment_3 == 'event' && segment_4}{!-- Event Member Listing --}
	{exp:user:stats dynamic="off"}
		{embed="sponsors/_email-event" sponsor_number="{member_admin_id}" sponsor_zipcode="{sponsor_zipcode}"}
	{/exp:user:stats}
	
{if:elseif segment_3 == 'deal' && segment_4}{!-- Deal Member Listing --}
	{exp:user:stats dynamic="off"}
		{embed="sponsors/_email-deal" sponsor_number="{member_admin_id}" sponsor_zipcode="{sponsor_zipcode}"}
	{/exp:user:stats}
	
{if:else}{!-- All Member Listing --}
	{exp:user:stats dynamic="off"}
		{embed="sponsors/_email-all" sponsor_number="{member_admin_id}" sponsor_zipcode="{sponsor_zipcode}"}
	{/exp:user:stats}
