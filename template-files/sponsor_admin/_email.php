...
function send_emails($mailing_list, $subject, $custom_message, $interest, $interest_line, $event, $rsvp_list, $full_list, $full_list_settings)
{
	...
	$message .= '
	  						<br /><br />
								{exp:user:stats dynamic="off"}{firstName} {lastName}<br />{exp:weblog:categories show="{sponsor_number}" weblog="{embed:channel}" style="linear"}{category_name}{/exp:weblog:categories}{/exp:user:stats}<br />
	...
	$headers .= 'From: {exp:user:stats dynamic="off"}{exp:weblog:categories show="{sponsor_number}" weblog="{embed:channel}" style="linear"}{category_name}{/exp:weblog:categories} <{username}>{/exp:user:stats}' . "\r\n";
	...
	print '<div class="heading clearafter">
				...
				<div class="grid23 clearafter">
							...
							<table>
								...
								<tr>
									...
									{if segment_3 && segment_4 == ""}
										...
										<textarea name="interest_line" class="hide"><br />You can update your interests in <a href="{path=members/settings}" style="font-size:10px;color:#FFFFFF;font-family:verdana;">Member Settings</a>.</textarea>
									{/if}
									...
									{if segment_3 == ""}
										...
										<textarea name="full_list_settings" class="hide"><br />You can update your preferences in <a href="{path=members/settings}" style="font-size:10px;color:#FFFFFF;font-family:verdana;">Member Settings</a>.</textarea>
									{/if}
										<p class="button-wrap">
					...
					<div class="sidebar right">
						...
						{exp:weblog:categories show="{sponsor_number}" weblog="{embed:channel}" style="linear"}{category_name}{/exp:weblog:categories}{/exp:user:stats}<br />
						<a href="http://www.newstartclub.com/">www.newstartclub.com</a></p>
					</div>
				</div><!-- /.grid23 -->';
...
function show_done()
{
	...
	print '</ul>
				<p class="button-wrap">
					<a href="{path=\'sponsor_admin/email-members\'}" class="super red button"><span>Back to Member List</span></a>
				</p>
				</div><!-- /.left -->
					<div class="sidebar right">
					</div>
