  ...
  <div class="grid23 clearafter">
    ...
    <div class="clear" id="recommendations">
        ...
        <ul>
          {if memberSleep != "11"}
            <li>
              <a href="/resources/living-better/rest">Get 7-8 hours of sleep nightly</a>
            </li>
          {/if}
          {if memberExercise != "13"}
            <li>
              <a href="/resources/living-better/exercise">Exercise regularly</a>
            </li>
          {/if}
          {if memberSmoking != "11"}
            <li>
              <a href="/resources/health-conditions/addictions">Don&rsquo;t smoke</a>
            </li>
          {/if}
          {if memberAlcohol != "11"}
            <li>
              <a href="/resources/health-conditions/addictions">Don&rsquo;t drink alcohol</a>
            </li>
          {/if}
          {if memberBreakfast != "10"}
            <li>
              <a href="/resources/media/recipe">Eat breakfast daily</a>
            </li>
          {/if}
          {if memberSnacking != "11"}
            <li>
              <a href="/resources/living-better/temperance">Don&rsquo;t eat between meals</a>
            </li>
          {/if}
          {if memberEmotional != "11"}
            <li>
              <a href="/resources/living-better/emotional-wellbeing">Boost your EQ</a>
            </li>
          {/if}
...
<div id="dashboard">
      ...
      <h2>Risk Level</h2>
      {exp:user:stats}
        <p><span class="one">&bull;</span> {if memberScoreTotal >= '90'}<strong>{/if}90-100 = Very Low{if memberScoreTotal >= '90'}</strong>{/if}</p>
        <p><span class="two">&bull;</span> {if memberScoreTotal >= '80' AND memberScoreTotal < '90'}<strong>{/if} 80-89 = Low{if memberScoreTotal >= '80' AND memberScoreTotal < '90'}</strong>{/if}</p>
        <p><span class="three">&bull;</span> {if memberScoreTotal < '80' AND memberScoreTotal >= '70'}<strong>{/if}70-79 = Medium{if memberScoreTotal < '80' AND memberScoreTotal >= '70'}</strong>{/if}</p>
        <p><span class="four">&bull;</span> {if memberScoreTotal < '70' AND memberScoreTotal >= '60'}<strong>{/if}60-69 = High{if memberScoreTotal < '70' AND memberScoreTotal >= '60'}</strong>{/if}</p>
        <p><span class="five">&bull;</span> {if memberScoreTotal < '60'}<strong>{/if}59 and lower = Very High{if memberScoreTotal < '60'}</strong>{/if}</p>
      {/exp:user:stats}
    </div><!--/.sidebar-->
  </div><!--/.grid23-->
</div><!-- /.body -->
